...
-(void)renderInLayerForYAxis:(CPTGraphHostingView *)hostView
	...
	float maxTicks = 5.0f;
	float range = [self niceNum:(maxPoint - minPoint)
												round:false];
	float tickSpacing = [self niceNum:range / (maxTicks - 1)
															round:true];
	float niceMin;
	float niceMax;
	...
	y.labelExclusionRanges = [NSArray arrayWithObjects:[CPTPlotRange plotRangeWithLocationDecimal:CPTDecimalFromInt(2)
																																									lengthDecimal:CPTDecimalFromInt(1)], nil];
	...
	// Use custom y-axis label so it will display text... instead of 1, 2, 3, 4
	...
	if (isNegative) {
		...
		[barPlot addAnimation:animation
									 forKey:@"barGrowth"];
