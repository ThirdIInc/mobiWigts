...
-(void)recreateWidget {
	...
	[barGraph renderInLayerForYAxis:barHostView
											 identifier:[NSString stringWithFormat:@"%d", barGraph.intGraphID]];
...
-(void)readDataValues {
	...
	for (int i = supportingMetricsStart; i < metricCount; i++) {
		...
		[dictSupportingMetrics setValue:[NSNumber numberWithDouble:[metricValue.rawValue doubleValue]]
...
-(void)readConstants {
	...
	strGraphTitle = [[[self.modelData arrayWithHeaderValueOfWholeRowByAxisType:ROW_AXIS
																																 andRowIndex:0] objectAtIndex:0] rawValue];
	
	// Always expect first metric value to be the graph ID
	barGraph.intGraphID = [[[[self.modelData arrayWithHeaderValueOfWholeRowByAxisType:ROW_AXIS
	...
	for (int i = 0; i < barGraph.intNoOfBars; i++) {
		...
		[barGraph.arrYAxisLabels addObject:[[[self.modelData arrayWithHeaderValueOfWholeRowByAxisType:ROW_AXIS
...
-(void)readFormattingInfo {
	...
	[barGraph.arrColors addObject:[self colorConvertor:[propertyGroup propertyByPropertySetID:FormattingFont
																																								 propertyID:FontFormattingColor]]];
	barGraph.ffGraph = [propertyGroup propertyByPropertySetID:FormattingFont
	...
	[barGraph.arrColors addObject:[self colorConvertor:[propertyGroup propertyByPropertySetID:FormattingFont
																																								 propertyID:FontFormattingColor]]];
	barGraph.fsGraphLabels = [propertyGroup propertyByPropertySetID:FormattingFont
																											 propertyID:FontFormattingSize];
...
-(UIView *)renderWidgetContainer:(CGRect)frameRect {
	...
	UILabel *graphTitle =[self createLableWithFrame:CGRectMake(0, 0, frameRect.size.width, 16)
																						 text:strGraphTitle
																				textColor:[barGraph.arrColors objectAtIndex:0]
																						 font:[UIFont fontWithName:barGraph.ffGraph
...
-(void)getSliderValues {
	...
	for (NSString *strKey in [[GoldmineReader getValue] allKeys]) {
		...
		[dictSupportingMetrics setValue:numValue
														 forKey:strKey];
...
-(NSDecimalNumber *)handleFormulae:(NSMutableString *)strFormula
													storeKey:(NSString *)strHeader {
	...
	if ([strFormula hasPrefix:@"#"]) {
		...
		strCalcValue = [objEvaluateFormulae evaluateFormula:strFormula
																						withMetrics:dictSupportingMetrics];
		
		[GoldmineReader setValue:[NSString stringWithFormat:@"%@",strCalcValue]
											forKey:strHeader];
		[dictSupportingMetrics setValue:[NSNumber numberWithDouble:[strCalcValue floatValue]]
														 forKey:strHeader];
