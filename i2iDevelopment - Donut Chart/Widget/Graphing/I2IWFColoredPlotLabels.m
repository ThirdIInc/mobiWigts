...
static const NSString *sUnits[] = { @"\0", @"M", @"MM", @"B"};
...
-(NSString *)stringForId:(id)obj {
    ...
    if ([obj floatValue] == 0.0) {
        ...
        convertedStr = [NSString stringWithFormat:@"%.2f%@", bytes, sUnits[exponent]];
        
    }
    else {
        
        convertedStr = [NSString stringWithFormat:@"%.1f%@", bytes, sUnits[exponent]];
