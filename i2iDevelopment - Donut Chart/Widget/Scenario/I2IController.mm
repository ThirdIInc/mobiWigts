...
-(void)setDefaultValues:(I2IControls *)objControls {
    ...
    if ([strTempControl isEqualToString:@"(null)"]) {
        ...
        if (number == nil) objControls.defaultCV = [[eval evaluateFormula:objControls.defaultCV withDictionary:[PlistData getValue]] stringValue];
        if ([objControls.suffix isEqualToString:@"%"]) {
            
            if ([objControls.step isEqualToString:@"1"]) objControls.defaultCV = [NSString stringWithFormat:@"%.2f", [objControls.defaultCV floatValue] / 100];
            else objControls.defaultCV = [NSString stringWithFormat:@"%.3f",[objControls.defaultCV floatValue] / 100];
