...
@implementation CustomLineSeries
...
@implementation CustomNumberLineXAxis
...
@implementation CustomNumberLineYAxis
-(NSString *)stringForId:(id)obj {
...
-(void)renderChart:(UIView *)hostView identifier:(NSString *)identifier{
    ...
    for (SChartDataPoint *dp in dataForPlot) {
        double x = [dp.xValue doubleValue];
        if (x < min) min = x;
        if (x > max) max = x;
    }
    max = max * 1.5;
    ...
    // Add x-axis
    SChartNumberAxis *xAxis = [[CustomNumberLineXAxis alloc] initWithRange:range];
    ...
    */
    ...
    SChartCategoryAxis *yAxis = [[CustomNumberLineYAxis alloc] init];
...
-(void)loadChartData {
    ...
    for (int barIndex = 0; barIndex < dataForPlot.count; barIndex++) {
        SChartDataPoint *dataPoint = [[SChartDataPoint alloc] init];
        
        dataPoint.xValue = [[dataForPlot objectAtIndex:barIndex] xValue];
        dataPoint.yValue = [[dataForPlot objectAtIndex:barIndex] yValue];
...
-(SChartSeries *)sChart:(ShinobiChart *)chart seriesAtIndex:(NSInteger)index {
    ...
    //CustomLineSeries *barSeries = [[CustomLineSeries alloc] init];
    SChartLineSeries *barSeries = [[SChartLineSeries alloc] init];
    barSeries.title = @"Test";
    barSeries.style.pointStyle.showPoints = NO;
